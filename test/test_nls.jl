...
@testset "NonlinearSolve: numerical agreement with baseline" begin
    ...
    sol_plain  = solve(prob_plain,  NewtonRaphson(), abstol = 1e-12)
    sol_opaque = solve(prob_opaque, NewtonRaphson(), abstol = 1e-12)

    @test sol_opaque.u[1] ≈ sol_plain.u[1] atol = 1e-10
    @test sol_opaque.u[1] ≈ 2.0 atol = 1e-10
end

# ---------------------------------------------------------------------------
@testset "NonlinearSolve: prob.p uniform across payloads" begin
    prob_q = NonlinearProblem(OpaqueResid!(QuadP, quad_kernel!),
                              [3.0], pack(QuadP(1.0, -3.0, 2.0)))
    prob_l = NonlinearProblem(OpaqueResid!(LineP, line_kernel!),
                              [0.0], pack(LineP(2.0, -4.0)))

    @test typeof(prob_q.p) === typeof(prob_l.p) === OpaqueParams

    sol_l = solve(prob_l, NewtonRaphson(), abstol = 1e-12)
    @test sol_l.u[1] ≈ 2.0 atol = 1e-10     # m*x + k = 0 => x = -k/m = 2
end
